...
#define SITTER_AUDIO_BUFFERSIZE 1024 // frames (samples/2 or bytes/4)
...
public:
  ...
  int whichbuf;
  volatile bool cbrun;
  volatile bool abort_cb;
  
  void populateBuffer();

  AudioManager() { chanv=0; chanc=0; mixbuf=0; chbuf=0; song=0; buf[0]=buf[1]=NULL; dummy=true; } // only used by DummyAudioManager
